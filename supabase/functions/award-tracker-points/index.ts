...
serve(async (req) => {
  ...
  try {
    ...
    const { user_id, activity_type, points_awarded, related_item_id, related_item_type } = await req.json()
    ...
    // --- Daily Task Logic ---
    const isDailyTask = ['daily_login', 'comment_made', 'quick_reviewer_task'].includes(activity_type);
    ...
    if (isDailyTask) {
      ...
      let taskDefinitionId = related_item_id;
      if (!taskDefinitionId) {
        const { data: taskDef, error: taskDefError } = await supabaseClient
          .from('daily_tasks_definitions')
          .select('id')
          .eq('task_name', activity_type === 'daily_login' ? 'Daily Check-In' :
                          activity_type === 'comment_made' ? 'Comment Comrade' :
                          activity_type === 'quick_reviewer_task' ? 'Quick Reviewer' : '')
          .single();

        if (taskDefError) {
          console.error(`Error fetching daily task definition for ${activity_type}:`, taskDefError);
      ...
      if (taskDefinitionId) {
        ...
        const { data: existingCompletion, error: checkCompletionError } = await supabaseClient

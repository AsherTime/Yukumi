...
// Define the view milestones and their corresponding points
...
serve(async (req) => {
  ...
  try {
    ...
    for (const milestone of VIEW_MILESTONES) {
      // Fetch posts that have reached this milestone's threshold
      // and have NOT yet awarded points for this specific milestone.
      ...
      console.log(`Found ${posts.length} posts to award ${milestone.identifier} points.`);
      ...
      for (const post of posts) {
        ...
        if (!awardResponse.ok) {
          ...
          console.log(`Successfully awarded ${milestone.points} points for post ${post.id} (${milestone.identifier}).`);
          awardedCount++;

          // Update the post's awarded_milestones to prevent re-awarding

...
export function TopNav({ children }: { children?: React.ReactNode }) {
  ...
  return (
    <div className={`overflow-hidden`}>
      ...
      <header className="fixed top-0 left-0 right-0 w-full z-50 bg-black border-b border-zinc-800 shadow-sm m-0 p-0">
        <nav className="flex items-center justify-between px-4 py-3 w-full">
          ...
          <div className="flex items-center justify-between w-full lg:w-auto pr-4 sm:px-6">
            ...
            {user && (
            <div className="flex items-center gap-4 ml-auto -mr-5 mt-2 lg:hidden">
            <Link href="/upload" className="text-white hover:text-gray-300 transition-colors -mr-4">
              <Upload />
            </Link>
            </div>
          )}
            {/* Notifications - shown on mobile only */}
      ...
      w-[90vw] sm:w-80 max-h-96 
      bg-black overflow-y-auto z-50 shadow-lg rounded-lg
      left-1/2 sm:left-auto sm:right-0 transform -translate-x-52 sm:translate-x-0
    "
                    >
              ...
              <button
                className="lg:hidden ml-auto mt-1"
        ...
        {/* Mobile menu */}
        {isMobileMenuOpen && (
          <div className="lg:hidden flex flex-col bg-black px-4 py-2 space-y-2">
            ...
            {navLinks.map((link) => (
              <Link key={link.label} href={link.href} className="text-white text-sm px-2 py-2 rounded hover:bg-zinc-800">
                {link.label}
              </Link>
            ))}
            ...
            <div className="pt-2 border-t border-zinc-700">
              {user ? (
                <>
                  <Link href={`/profile/${user.id}`} className="block py-2 text-white">
                    Profile
                  </Link>
                  <Link href={`/settings`} className="block py-2 text-white">
                    Settings
                  </Link>
                  <div
                    className="text-red-500 hover:bg-zinc-800 cursor-pointer block py-2"
                    onClick={() => {
                      const logoutButton = document.querySelector('[data-logout-button]');
                      if (logoutButton) {
                        (logoutButton as HTMLButtonElement).click();
                ...
                <>
                  ...
                  <Link href="/auth/register" className="block py-2 text-white">Register</Link>
                </>
              )}
            </div>
          </div>
